...
def import_module_directly(module_name, file_path):
    ...
    if module_name == "system":
        ...
        if os.path.exists(compendium_path):
            compendium_spec = importlib.util.spec_from_file_location(
                "grimoire_runner.models.compendium", compendium_path
            )
...
class TestModelDefinitionComplete:
    ...
    def test_model_all_fields(self):
        ...
        attrs = {
            ...
            "class": AttributeDefinition(
                type="str", enum=["fighter", "wizard", "rogue"], required=True
            ),
        }
        ...
        validations = [
            ...
            ValidationRule(expression="len(name) > 0", message="Name cannot be empty"),
        ]
    ...
    def test_get_attribute_nested(self):
        ...
        model = ModelDefinition(
            id="nested_model", name="Nested Model", attributes=nested_attrs
        )
...
class TestSystemDefinitionComplete:
    ...
    def test_system_validation(self):
        ...
        valid_system = SystemDefinition(
            kind="system", id="valid_system", name="Valid System"
        )
        ...
        invalid_system = SystemDefinition(
            kind="invalid", id="invalid_system", name="Invalid System"
        )
...
class TestObservableComplete:
    ...
    def test_derived_field_manager_complex_scenarios(self):
        ...
        dollar_expr = (
            "$character.stats.strength + $character.stats.dexterity + $base_bonus"
        )
    ...
    def test_derived_field_manager_dependency_graph(self):
        ...
        manager.derived_fields["strength_bonus"] = DependencyInfo(
            derived="{{ (strength - 10) / 2 }}", dependencies={"strength"}
        )
