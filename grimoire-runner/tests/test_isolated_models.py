...
class TestModelDefinition:
    ...
    def test_model_initialization(self):
        ...
        model = ModelDefinition(
            id="test_model", name="Test Model", description="A test model"
        )
    ...
    def test_attribute_definition(self):
        ...
        attr = AttributeDefinition(
            type="int", default=10, range="1..20", description="Character strength"
        )
    ...
    def test_validation_rule(self):
        ...
        rule = ValidationRule(
            expression="strength >= 1", message="Strength must be at least 1"
        )
...
class TestFlowDefinition:
    ...
    def test_flow_initialization(self):
        ...
        flow = FlowDefinition(
            id="test_flow", name="Test Flow", description="A test flow"
        )
    ...
    def test_flow_get_step(self):
        ...
        flow = FlowDefinition(id="test_flow", name="Test Flow", steps=[step])
...
class TestStepDefinition:
    ...
    def test_step_with_actions(self):
        ...
        step = StepDefinition(id="action_step", name="Action Step", actions=actions)
...
class TestCompendiumDefinition:
    ...
    def test_compendium_get_entry(self):
        ...
        entries = {"test_entry": {"name": "Test Entry", "description": "A test entry"}}
...
class TestObservablePattern:
    ...
    def test_dependency_info(self):
        ...
        deps = DependencyInfo(
            derived="{{ strength + dexterity }}", dependencies={"strength", "dexterity"}
        )
...
class TestSourceDefinition:
    ...
    def test_source_initialization(self):
        ...
        source = SourceDefinition(kind="source", name="Test Source", id="test_source")
    ...
    def test_source_validation(self):
        ...
        # Valid source
        valid_source = SourceDefinition(kind="source", name="Valid Source")
        errors = valid_source.validate()
        assert len(errors) == 0

        # Invalid kind
        invalid_source = SourceDefinition(kind="invalid", name="Invalid Source")
        errors = invalid_source.validate()
        assert len(errors) > 0
        assert any("kind must be 'source'" in error for error in errors)

        # Missing name
        no_name_source = SourceDefinition(kind="source", name="")
...
class TestModelIntegration:
    ...
    def test_flow_with_model_actions(self):
        ...
        step = StepDefinition(
            id="setup_character", name="Setup Character", actions=actions
        )
        flow = FlowDefinition(
    ...
    def test_model_with_attributes_and_compendium(self):
        ...
        from grimoire_runner.models.compendium import CompendiumDefinition
        from grimoire_runner.models.model import AttributeDefinition, ModelDefinition
        ...
        compendium = CompendiumDefinition(
            ...
            model="character",
            entries={
                "guard": {"name": "Town Guard", "level": 2, "strength": 14},
                "wizard": {"name": "Village Wizard", "level": 5, "strength": 8},
            },
        )
