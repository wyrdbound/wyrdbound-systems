...
class TestDiceExecutor:
    ...
    def test_simple_dice_roll(self):
        ...
        step = MockStep(step_type="dice_roll", roll="1d6", output="result")

        # Mock the dice integration
        with patch.object(
            self.executor.dice_integration, "roll_expression"
        ) as mock_roll:
    ...
    def test_dice_roll_without_output(self):
        ...
        step = MockStep(step_type="dice_roll", roll="1d4")  # No output specified
    ...
    def test_dice_roll_missing_roll_param(self):
        ...
        step = MockStep(step_type="dice_roll", output="result")  # No roll specified
...
class TestTableExecutor:
    ...
    def test_table_executor_with_no_tables(self):
        ...
        step = MockStep(step_type="table_roll", output="result")
    ...
    def test_table_executor_without_system(self):
        ...
        step = MockStep(
            step_type="table_roll", tables=[{"table": "test_table"}], output="result"
        )
...
class TestFlowExecutor:
    ...
    def test_completion_step(self):
        ...
        step = MockStep(step_type="completion", message="Flow completed successfully")
    ...
    def test_flow_call_structure(self):
        ...
        step = MockStep(step_type="flow_call", flow="sub_flow", output="sub_result")
...
class TestExecutorIntegration:
    ...
    def test_executor_result_consistency(self):
        ...
        for executor in executors:
            step = MockStep(
                step_type="dice_roll"
                if isinstance(executor, DiceExecutor)
                else "player_choice"
                if isinstance(executor, ChoiceExecutor)
                else "table_roll"
    ...
    def test_executor_context_isolation(self):
        ...
        # Mock dice results
        with patch.object(
            dice_executor.dice_integration, "roll_expression"
        ) as mock_roll:
            from grimoire_runner.integrations.dice_integration import DiceResult
