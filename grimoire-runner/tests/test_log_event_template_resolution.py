...
def test_log_event_template_resolution():
    ...
    context = MagicMock()
    context.resolve_template.return_value = (
        "Input: Hello World, Variable: test_value, Result: success"
    )
    ...
    with patch.object(
        logging.getLogger("grimoire_runner.executors.action_strategies"), "debug"
    ) as mock_logger:
        ...
        context.resolve_template.assert_called_once_with(
            "Input: {{ inputs.test_input }}, Variable: {{ variables.test_var }}, Result: {{ result }}"
        )

        # Verify the logger was called with the resolved template
        mock_logger.assert_called_once_with(
            "Event: test_event - Input: Hello World, Variable: test_value, Result: success"
        )
...
def test_log_event_non_string_data():
    ...
    with patch.object(
        ...
        action_data = {"type": "test_event", "data": {"key": "value", "number": 42}}
        ...
        mock_logger.assert_called_once_with(
            "Event: test_event - {'key': 'value', 'number': 42}"
        )
