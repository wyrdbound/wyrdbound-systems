...
def _execute_with_rich_console(
    ...
    try:
        ...
        info_table.add_row(
            ...
            flow_obj.resolve_description(temp_context) or "[dim]No description[/dim]",
        )
        ...
        if no_progress:
            ...
            result = _execute_flow_with_progress(
                engine, flow, context, system, interactive
            )
...
def _execute_flow_with_progress(
    engine, flow_id, context, system, interactive_mode=False
):
        ...
        try:
            ...
            for step_result in engine.step_through_flow(flow_id, context, system):
                ...
                if step_result.requires_input and interactive_mode:
                    progress.stop()
                    choice_result = _handle_interactive_choice(
                        step_result, context, engine, step
                    )
                    ...
                    console.print(
                        f"[yellow]⚠ Step '{step_name}' requires user input (skipped in non-interactive mode)[/yellow]"
                    )
                    break
            ...
            progress.update(
                task,
                description="[bold green]Flow completed![/bold green]",
    ...
    stopped_for_input = (
        ...
        and step_results[-1].requires_input
        and not interactive_mode
    )

    # Success if either all steps completed successfully OR we stopped for user input
    success = all_completed_steps_successful and (
        completed_all_steps or stopped_for_input
    )
    ...
    return FlowResult(
        ...
        error=error,
        completed_at_step=step_results[-1].step_id
            ...
            try:
                    ...
                    while True:
                        try:
                            user_input = typer.prompt(
                                f"Enter choice number for selection {selection_num + 1}"
                            )
                            ...
                            if 0 <= choice_num < len(available_choices):
                                ...
                                console.print(
                                    f"[green]✓ Selected: {selected_choice.label}[/green]"
                                )
                                ...
                                console.print(
                                    "[red]Invalid choice number. Please try again.[/red]"
                                )
                ...
                if step.actions:
                    for action in step.actions:
                        engine._execute_single_action(
                            action, context, {"selected_items": selected_ids}
                        )
                ...
                try:
                    ...
                    if 0 <= choice_num < len(step_result.choices):
                        ...
                        choice_result = choice_executor.process_choice(
                            selected_choice.id, step, context
                        )
                        console.print(
                            f"[green]✓ Selected: {selected_choice.label}[/green]"
                        )
...
def _display_success_results(result, system, flow_obj):
    ...
    summary_table.add_row(
        ...
        f"{sum(1 for sr in result.step_results if sr.success)}/{len(result.step_results)}",
    )
    ...
    if result.step_results:
        ...
        for sr in result.step_results:
            ...
            if sr.data:
                details = ", ".join(
                    [f"{k}: {v}" for k, v in sr.data.items() if k not in ["skipped"]]
                )
...
def _display_failure_results(result):
    ...
    console.print(
        Panel.fit("[bold red]✗ Flow Execution Failed[/bold red]", border_style="red")
    )
    console.print()
    ...
    if result.step_results:
        ...
        for sr in result.step_results:
            ...
            error = sr.error or ""
            step_table.add_row(
                sr.step_id, status, error[:50] + "..." if len(error) > 50 else error
            )
                    ...
                    for i, item in enumerate(items):
                        try:
                            if hasattr(item, "items"):  # Dictionary-like
                                ...
                                _add_dict_to_tree(
                                    item_branch, item, max_depth, current_depth + 2
                                )
                            else:
