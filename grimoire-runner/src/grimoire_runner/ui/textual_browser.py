...
class CompendiumDetailScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            ...
            Rule(),
            TextArea(self._format_entry_data(), read_only=True, id="entry-content"),
...
class CompendiumBrowserApp(App):
    ...
    def _create_category_tree(self) -> Tree:
        ...
        for comp_id, compendium in self.compendiums.items():
            comp_node = tree.root.add(
                comp_id, data={"type": "compendium", "id": comp_id}
            )
    ...
    def _resolve_entry_with_model(
        self, compendium: CompendiumDefinition, entry_data: dict[str, Any]
    ) -> dict[str, Any]:
            ...
            for attr_name, attr_def in all_attributes.items():
                if attr_name not in resolved_data:
                    ...
                    if hasattr(attr_def, "default") and attr_def.default is not None:
                        ...
                        isinstance(attr_def, dict)
                        and "default" in attr_def
    ...
    def _resolve_model_attributes(
        self, model_id: str, visited: set = None
    ) -> dict[str, Any]:
        ...
        if hasattr(model, "extends") and model.extends:
            for parent_model in model.extends:
                parent_attributes = self._resolve_model_attributes(
                    parent_model, visited
                )
                ...
                if isinstance(value, str | int | float | bool):
                    ...
                    if key.lower() == "tags":
                        ...
                        if len(value) == 0:
                            ...
                            lines.append(
                                f"[dim]{key}:[/dim] [{preview_items}, ...{len(value) - 3} more]"
                            )
                    ...
                    if len(value) == 0:
                        ...
                        if len(value) > 1:
                            lines.append(
                                f"[dim]{key}:[/dim] {{{preview}, ...{len(value) - 1} more}}"
                            )
                        else:
