...
class ChoiceModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="choice-container"):
            ...
            if self.is_multiple:
                yield Label(
                    f"Selection {self.current_selection + 1} of {self.selection_count}"
                )
        ...
        if event.button.id == "confirm":
            ...
            if radio_set.pressed_button:
                ...
                selected_choice = next(
                    (c for c in self.step_result.choices if c.id == selected_id), None
                )
                ...
                if self.is_multiple:
                    ...
                    if self.current_selection < self.selection_count:
                        ...
                        remaining_choices = [
                            c
                            for c in self.step_result.choices
                            if c not in self.selected_choices
                        ]
...
class FlowExecutionApp(App):
    ...
    async def load_system(self) -> None:
        ...
        try:
            ...
            if not self.flow_obj:
                self.query_one("#step-info").update(
                    Text("Flow not found!", style="bold red")
                )
                return
    ...
    async def execute_flow(self) -> None:
        ...
        try:
            ...
            while current_step_id:
                ...
                self.query_one("#step-info").update(
                    Text(
                        f"Step {step_num}/{self.total_steps}: {step_name}", style="cyan"
                    )
                )

                # Log step start
                self.log_message(
                    f"[cyan]▶[/cyan] Starting Step {step_num}: {step_name}"
                )
                ...
                try:
                    success, next_step_id = await self.execute_single_step(
                    ...
                    self.log_message(
                        f"[red]Error executing step {current_step_id}: {e}[/red]"
                    )
                    step_result = StepResult(
                        step_id=current_step_id, success=False, error=str(e)
                    )
            ...
            # Create final result
            success = (
                all(sr.success for sr in self.step_results)
                and step_num >= self.total_steps
            )
            ...
            self.current_result = FlowResult(
                ...
                error=self.step_results[-1].error
                if self.step_results and not self.step_results[-1].success
                else None,
                completed_at_step=self.step_results[-1].step_id
                if self.step_results and not self.step_results[-1].success
                else None,
            )
    ...
    async def execute_single_step(
        ...
        if step_result.requires_input and self.interactive:
            ...
            self.log_message("[yellow]User input completed for step[/yellow]")

        elif step_result.requires_input and not self.interactive:
            self.log_message(
                f"[yellow]⚠[/yellow] Step '{step_name}' requires user input (skipped in non-interactive mode)"
            )
            step_result.success = False
            step_result.error = (
                "User input required but not available in non-interactive mode"
            )
        ...
        if step_result.success:
            ...
            self.query_one("#step-info").update(
                Text(
                    f"✓ Step {step_num}/{self.total_steps}: {step_name}", style="green"
                )
            )
        else:
            self.log_message(
                f"[red]✗[/red] Failed Step {step_num}: {step_name} - {step_result.error}"
            )
            self.query_one("#step-info").update(
                Text(f"✗ Step {step_num}/{self.total_steps}: {step_name}", style="red")
            )
    ...
    async def handle_user_input(self, step_result: StepResult, step) -> None:
        ...
        if step_result.choices:
            ...
            self.query_one("#step-info").update(
                Text(
                    f"⏳ Waiting for user input: {step.name or step.id}", style="yellow"
                )
            )
            ...
            if is_multiple:
                ...
                self.log_message(
                    f"[cyan]Please select {selection_count} choices:[/cyan]"
                )
                ...
                if step.actions:
                    for action in step.actions:
                        self.engine._execute_single_action(
                            action, self.context, {"selected_items": selected_ids}
                        )
                ...
                self.log_message(
                    f"[green]Selected: {', '.join(selected_labels)}[/green]"
                )

            else:
                # Single selection
                self.log_message("[cyan]Please select one choice:[/cyan]")
                for i, choice in enumerate(step_result.choices, 1):
                    self.log_message(f"  {i}. {choice.label}")
            ...
            self.query_one("#step-info").update(
                Text(f"✓ Choice made for: {step.name or step.id}", style="green")
            )

    async def push_screen_wait(self, modal):
        """Push a modal screen and wait for its result."""
        self.log_message("[dim]Pushing modal screen...[/dim]")
    ...
    async def display_results(self) -> None:
        ...
        if self.current_result.success:
            self.query_one("#step-info").update(
                Text("✓ Flow execution completed successfully!", style="bold green")
            )
...
"""
            ...
            if self.current_result.variables:
                variables_pane = TabPane(
                    "Variables", Pretty(self.current_result.variables), id="variables"
                )
                results_tabs.add_pane(variables_pane)

        else:
            self.query_one("#step-info").update(
                Text("✗ Flow execution failed", style="bold red")
            )
...
"""
    ...
    def action_save(self) -> None:
        ...
        if not self.current_result or not self.output_path:
            self.log_message(
                "[yellow]No results to save or output path not specified[/yellow]"
            )
            return
            ...
            output_data = {
                ...
                "execution_summary": {
                    ...
                    "successful_steps": sum(
                        1 for sr in self.current_result.step_results if sr.success
                    ),
                    "failed_steps": sum(
                        1 for sr in self.current_result.step_results if not sr.success
                    ),
                },
            }
