...
class InteractiveREPL:
    ...
    def _execute_full_flow(self) -> None:
        ...
        try:
            result = self.engine.execute_flow(
                self.current_flow, self.context, self.system
            )

            if result.success:
                console.print("[bold green]✓ Flow completed successfully[/bold green]")
                ...
                if result.outputs:
                    ...
                    for output_id, output_value in result.outputs.items():
                        console.print(
                            Panel(
                                str(output_value), title=output_id, border_style="green"
                            )
                        )
            else:
                console.print("[bold red]✗ Flow execution failed[/bold red]")
        ...
        if flows:
            ...
            for flow_id in flows:
                ...
                resolved_description = (
                    flow.resolve_description(temp_context) if flow.description else "-"
                )
                table.add_row(
