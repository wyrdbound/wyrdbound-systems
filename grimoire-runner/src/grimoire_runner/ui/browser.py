...
class CompendiumBrowser:
    ...
    def browse_compendium(self, compendium_name: str, search_term: Optional[str] = None) -> None:
        ...
        console.print(f"[dim]Model: {compendium.model} | Entries: {len(compendium.entries)}[/dim]\n")
        ...
        if search_term:
            ...
            console.print(f"[yellow]Filtered by '{search_term}': {len(entries_to_show)} matches[/yellow]\n")
        ...
        for entry_id, entry_data in list(entries_to_show.items())[:20]:  # Limit to 20 entries
            row = [entry_id]
    ...
    def _find_common_attributes(self, entries: Dict[str, Any]) -> List[str]:
        ...
        priority_attrs = ['name', 'display_name', 'description', 'type', 'cost', 'weight', 'damage']
    ...
    def _show_detailed_entries(self, entries: Dict[str, Any]) -> None:
        ...
        for entry_id, entry_data in entries.items():
            ...
            if len(entries) > 5:
                try:
                    console.print("[dim](Press Enter for next entry, Ctrl+C to stop)[/dim]")
                    input()
    ...
    def search_across_compendiums(self, search_term: str) -> None:
        ...
        console.print(f"[bold blue]Searching all compendiums for '{search_term}'[/bold blue]\n")
        ...
        for comp_name in self.system.list_compendiums():
            ...
            if matches:
                ...
                for entry_id, entry_data in list(matches.items())[:3]:
                    display_name = entry_data.get('display_name', entry_data.get('name', entry_id))
    ...
    def analyze_compendium(self, compendium_name: str) -> None:
        ...
        # Basic stats
        console.print(f"[cyan]Basic Statistics:[/cyan]")
        ...
        for attr in sorted(all_attrs):
            ...
            console.print(f"  {attr}: {coverage}/{len(compendium.entries)} ({percentage:.1f}%)")
        
        console.print(f"\n[cyan]Unique attributes:[/cyan] {len(all_attrs)}")
        
        # Value analysis for common attributes
        self._analyze_attribute_values(compendium.entries, ['cost', 'weight', 'damage', 'type'])
    
    def _analyze_attribute_values(self, entries: Dict[str, Any], attributes: List[str]) -> None:
