...
class FlowExecutionScreen(Screen):
    ...
    def execute_flow(self) -> None:
        ...
        try:
            ...
            for step_result in self.engine.step_through_flow(
                self.flow_id, self.context, self.system
            ):
                step_num += 1
                ...
                if step_result.success:
                    log.write_line("  ✓ Completed successfully")
                    if step_result.data:
                        ...
                        if "breakdown" in step_result.data:
                            log.write_line(
                                f"  Breakdown: {step_result.data['breakdown']}"
                            )
                else:
            ...
            if all_successful and completed_all:
                ...
                if not all_successful:
                    failed_step = next(
                        (sr for sr in step_results if not sr.success), None
                    )
                    if failed_step:
        ...
        if event.button.id == "execute-flow":
            ...
            if selected:
                ...
                self.app.push_screen(
                    FlowExecutionScreen(self.system, flow_id, self.engine)
                )
...
class GrimoireApp(App):
    ...
    def __init__(self, system_path: Path | None = None, auto_flow: str | None = None):
        super().__init__()
        ...
        if self.system:
            if self.auto_flow:
                ...
                if self.auto_flow in self.system.flows:
                    self.push_screen(
                        FlowExecutionScreen(self.system, self.auto_flow, self.engine)
                    )
                else:
...
def run_textual_app(
    system_path: Path | None = None, auto_flow: str | None = None
) -> None:
