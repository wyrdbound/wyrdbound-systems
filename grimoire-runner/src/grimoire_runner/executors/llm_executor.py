...
class LLMExecutor(BaseStepExecutor):
    ...
    def _create_cleanup_template(
        self, raw_response: str, schema: dict, validation_errors: list[str]
    ) -> str:
...
Please provide ONLY the corrected JSON object with no additional text or formatting."""
    ...
    def _attempt_llm_call_with_validation(
        self, prompt_template: str, prompt_data: dict, validation_config, settings: dict
        ...
        logger.debug(
            f"Starting validation-aware LLM generation: max_attempts={max_attempts}, cleanup_enabled={cleanup_enabled}"
        )
        ...
        for attempt in range(max_attempts):
            ...
            if attempt == 0:
                ...
                logger.debug("Retry attempt with modified template")
            ...
            if not json_extracted:
                ...
                if attempt < max_attempts - 1 and cleanup_enabled:
                    # Try cleanup call
                    cleanup_template = """The following response could not be parsed as JSON:
...
Please provide the same information as a valid JSON object with no additional text or formatting."""
                    ...
                    cleanup_json, cleanup_extracted = self._extract_json_from_response(
                        cleanup_response
                    )
            ...
            if validation_config.type == "json_schema" and hasattr(
                validation_config, "schema"
            ):
                logger.debug("Validating against JSON schema")
                is_valid, errors = self._validate_json_schema(
                    json_data, validation_config.schema
                )
                ...
                if not is_valid:
                    ...
                    if attempt < max_attempts - 1 and cleanup_enabled:
                        ...
                        cleanup_template = self._create_cleanup_template(
                            raw_response, validation_config.schema, errors
                        )
                        logger.debug("Attempting cleanup call for schema validation")
                        cleanup_response = self.llm_integration.generate_content(
                            prompt_template=cleanup_template, context={}, **settings
                        )
                        logger.debug(f"Schema cleanup response:\n{cleanup_response}")

                        cleanup_json, cleanup_extracted = (
                            self._extract_json_from_response(cleanup_response)
                        )

                        if cleanup_extracted:
                            cleanup_valid, cleanup_errors = self._validate_json_schema(
                                cleanup_json, validation_config.schema
                            )
                            if cleanup_valid:
    ...
    def execute(
        ...
        try:
            ...
            if step.validation:
                logger.debug(
                    f"Validation configured: type={step.validation.type}, max_attempts={step.validation.max_attempts}"
                )

                # Use validation-aware generation
                result_json, success, attempts, raw_response, errors = (
                    self._attempt_llm_call_with_validation(
                        prompt_template,
                        prompt_data,
                        step.validation,
                ...
                logger.debug(
                    f"Validation results: success={success}, attempts={attempts}"
                )
                if not success:
                ...
                if success:
                    ...
                    logger.debug(
                        f"Handling validation failure with strategy: {on_failure}"
                    )

                    if on_failure == "fallback" and hasattr(
                        step.validation, "fallback_value"
                    ):
                        context.set_variable("llm_fallback_used", True)
                        context.set_variable(
                            "llm_result", step.validation.fallback_value
                        )
                        generated_content = step.validation.fallback_value
                        logger.debug(
                            f"Using fallback value: {step.validation.fallback_value}"
                        )
