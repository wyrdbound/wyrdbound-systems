...
class DiceExecutor(BaseStepExecutor):
    ...
    def _execute_dice_roll(self, step: "StepDefinition", context: "ExecutionContext") -> "StepResult":
        ...
        try:
            ...
            # Roll the dice
            result = self.dice_integration.roll_expression(str(roll_expression), modifiers)
    ...
    def _execute_dice_sequence(self, step: "StepDefinition", context: "ExecutionContext") -> "StepResult":
        ...
        try:
            for item in sequence.items:
                ...
                if sequence.actions:
                    for action in sequence.actions:
                        self._execute_sequence_action(action, context, item, result.total)
            ...
            return StepResult(
                step_id=step.id,
                success=False,
                error=f"Dice sequence failed: {e}"
            )
    
    def _execute_sequence_action(self, action: Dict[str, Any], context: "ExecutionContext", item: str, result: int) -> None:
