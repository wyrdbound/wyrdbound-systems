...
class TableExecutor(BaseStepExecutor):
    ...
    def execute(self, step: "StepDefinition", context: "ExecutionContext", system: "System") -> "StepResult":
        ...
        try:
            ...
            return StepResult(
                step_id=step.id,
                success=False,
                error=f"Table roll failed: {e}"
            )
    
    def _execute_table_roll(self, table_roll, context: "ExecutionContext", system: "System"):
        ...
        for _ in range(count):
            ...
            if table_roll.actions:
                self._execute_table_actions(table_roll.actions, context, table_result, results)
    ...
    def _execute_table_actions(self, actions: List[Any], context: "ExecutionContext", result: Any, all_results: List[Any]) -> None:
    ...
    def _execute_table_action(self, action, context: "ExecutionContext", result: Any) -> None:
