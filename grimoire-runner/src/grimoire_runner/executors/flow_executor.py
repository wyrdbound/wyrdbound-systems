...
class FlowExecutor(BaseStepExecutor):
    ...
    def _execute_action(self, action: dict, context: "ExecutionContext") -> None:
        ...
        if action_type == "set_value":
            ...
            if self._is_list_append_operation(value_template):
                resolved_value = self._handle_list_append_operation(value_template, context)
                logger.info(f"Handled list append programmatically: {type(resolved_value).__name__} with {len(resolved_value) if hasattr(resolved_value, '__len__') else 'N/A'} items")
            else:
                # Resolve the value template normally
                try:
                    resolved_value = context.resolve_template(value_template)
                    logger.info(f"Resolved template to: {type(resolved_value).__name__} = {resolved_value}")
                except Exception as e:
    ...
    def _handle_list_append_operation(self, template: str, context) -> list:
        ...
        logger.info(f"Parsed list append: base_list='{base_list_path}', item='{item_path}'")
        ...
        try:
            ...
            if base_list_path == "inputs.character.inventory":
                ...
                logger.info(f"Direct access to inventory: {type(current_list).__name__} with {len(current_list) if isinstance(current_list, list) else 'non-list'} items")
            else:
            ...
            if current_list is None:
                current_list = []
            elif isinstance(current_list, str) and current_list.startswith("[") and current_list.endswith("]"):
                ...
                logger.warning(f"Base list is string representation, starting fresh: {current_list}")
                current_list = []
            elif not isinstance(current_list, list):
                logger.warning(f"Base list is not a list type ({type(current_list)}), starting fresh")
                current_list = []
            else:
                # Make a copy to avoid modifying the original
                current_list = list(current_list)
        except Exception as e:
            logger.warning(f"Could not resolve base list '{base_list_path}': {e}, starting with empty list")
            current_list = []
        ...
        try:
            ...
            if item_path == "inputs.item":
                ...
                logger.info(f"Item to append (direct): {type(item_to_append).__name__} = {item_to_append}")
            else:
                ...
                if item_to_append == "None":
                    item_to_append = None
                logger.info(f"Item to append (template): {type(item_to_append).__name__} = {item_to_append}")
        except Exception as e:
        ...
        if item_to_append is None:
            logger.info(f"Skipping None item, not appending to list")
            return current_list
