...
from jinja2 import Environment, BaseLoader, TemplateError, StrictUndefined, meta
import logging
...
class TemplateHelpers:
    ...
    def _setup_custom_functions(self) -> None:
        ...
        self.env.filters['snake_case'] = lambda x: re.sub(r'[^a-zA-Z0-9]', '_', str(x)).lower()
        self.env.filters['kebab_case'] = lambda x: re.sub(r'[^a-zA-Z0-9]', '-', str(x)).lower()
        self.env.filters['upper_first'] = lambda x: str(x)[:1].upper() + str(x)[1:] if x else ""
