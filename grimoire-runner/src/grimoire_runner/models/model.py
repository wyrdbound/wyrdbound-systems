...
class AttributeDefinition:
    ...
    range: Optional[str] = None  # e.g., "1..20", "1..", etc.
    enum: Optional[List[str]] = None
    derived: Optional[str] = None  # Formula for derived attributes
    required: bool = True
...
class ModelDefinition:
    ...
    attributes: Dict[str, Union[AttributeDefinition, Dict[str, Any]]] = field(default_factory=dict)
    validations: List[ValidationRule] = field(default_factory=list)
    
    def get_attribute(self, attr_path: str) -> Optional[AttributeDefinition]:
    ...
    def get_all_attributes(self, include_inherited: bool = True) -> Dict[str, AttributeDefinition]:
    ...
    def _validate_attribute_value(self, attr_path: str, value: Any, attr_def: AttributeDefinition) -> List[str]:
        ...
        if attr_def.enum and value not in attr_def.enum:
            errors.append(f"Attribute '{attr_path}' must be one of: {', '.join(attr_def.enum)}")
        
        return errors
    
    def _validate_range(self, attr_path: str, value: Union[int, float], range_spec: str) -> List[str]:
        ...
        try:
            ...
            errors.append(f"Invalid range specification '{range_spec}' for attribute '{attr_path}'")
