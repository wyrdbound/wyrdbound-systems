...
from .services.ui_service import GrimoireUIService
from .utils.logging import setup_logging


class StepDisplayFormatter:
    """Handles unified formatting of step data across all step types."""
    ...
    def format_display_value(self, path: str, value: Any) -> str:
        ...
        if isinstance(value, dict):
            ...
            if isinstance(value, str):
                display_value = (
                    f'"{value}"' if len(str(value)) < 50 else f'"{str(value)[:47]}..."'
                )
            else:
...
class SimpleEventCLI:
    ...
    def _handle_step_started(self, sender=None, **kwargs):
        ...
        data = kwargs.get("data")
        self.logger.debug("Received signal: step_started")
        step = data.step_info
    ...
    def _handle_step_completed(self, sender=None, **kwargs):
        ...
        data = kwargs.get("data")
        self.logger.debug("Received signal: step_completed")
        step = data.step_info
        step_number = data.step_number
        ...
        self.formatter.display_step_data(step, data.step_data)
    ...
    def _handle_log_message(self, sender=None, **kwargs):
        ...
        data = kwargs.get("data")
        self.logger.debug("Received signal: log_message")
    ...
    def _handle_input_required(self, sender=None, **kwargs):
        ...
        data = kwargs.get("data")
        self.logger.debug("Received signal: input_required")
    ...
    def _handle_choice_required(self, sender=None, **kwargs):
        ...
        data = kwargs.get("data")
        self.logger.debug("Received signal: choice_required")
    ...
    def _handle_single_choice(self, data):
        ...
        try:
            while True:
                try:
                    choice_input = input(
                        f"\nEnter choice (1-{len(data.choices)}): "
                    ).strip()
                    ...
                    if 0 <= choice_index < len(data.choices):
                        ...
                        print(
                            f"❌ Invalid choice. Please enter a number between 1 and {len(data.choices)}"
                        )
                except ValueError:
        ...
        try:
            for selection_num in range(selection_count):
                ...
                while True:
                    try:
                        choice_input = input(
                            f"\nEnter choice (1-{len(available_choices)}): "
                        ).strip()
                        ...
                        if choice_input in choice_map:
                            ...
                            print(
                                f"❌ Invalid choice. Please choose from: {choices_text}"
                            )
                    except ValueError:
                        print("❌ Please enter a valid number")
    ...
    def _handle_flow_completed(self, sender=None, **kwargs):
        ...
        data = kwargs.get("data")
        self.logger.debug("Received signal: flow_completed")
    ...
    def _handle_error_occurred(self, sender=None, **kwargs):
        ...
        data = kwargs.get("data")
        self.logger.debug("Received signal: error_occurred")
        self.execution_complete = True
        self.execution_successful = False
    ...
    def _handle_flow_cancelled(self, sender=None, **kwargs):
        ...
        data = kwargs.get("data")
        self.logger.debug("Received signal: flow_cancelled")
        self.execution_complete = True
        self.execution_successful = False
    ...
    def _handle_display_value(self, sender=None, **kwargs):
        ...
        data = kwargs.get("data")
        self.logger.debug("Received signal: display_value")
    ...
    def load_system(self, system_path: Path) -> bool:
        ...
        try:
            ...
            self.ui_service.load_system(system_path)
            return True
        ...
        try:
            ...
            if flows:
                ...
                for flow in flows:
                    ...
                    print(
                        f"    Steps: {flow.step_count}, Inputs: {'Yes' if flow.requires_inputs else 'No'}, Outputs: {'Yes' if flow.produces_outputs else 'No'}"
                    )
            else:
    ...
    def execute_flow(
        self, system_id: str, flow_id: str, inputs: dict[str, Any] = None
    ) -> bool:
...
def execute_flow_interactively(engine, flow_id: str, context, system) -> bool:
...
Examples:
  ...
  grimoire --list-flows system.yaml
    ...
    parser.add_argument(
        "system_path", type=Path, help="Path to GRIMOIRE system directory or YAML file"
    )

    parser.add_argument(
        "flow_id", nargs="?", help="ID of the flow to execute (omit to list flows)"
    )
    ...
    parser.add_argument(
        "--list-flows", action="store_true", help="List available flows in the system"
    )

    parser.add_argument(
        "--inputs", type=Path, help="YAML file containing initial input values"
    )
    ...
    try:
        ...
        system_id = (
            args.system_path.stem
            if args.system_path.is_file()
            else args.system_path.name
        )
