...
def publish_session_created(session_id: str, system_id: str, flow_id: str) -> None:
    ...
    data = SessionCreatedData(
        session_id=session_id, system_id=system_id, flow_id=flow_id
    )
    session_created.send(data=data)


def publish_flow_started(
    session_id: str,
    flow_id: str,
    flow_name: str,
    system_id: str,
...
def publish_step_started(
    session_id: str, step_info: StepInfo, step_number: int
) -> None:
    """Publish a step started event."""
    data = StepStartedData(
        session_id=session_id, step_info=step_info, step_number=step_number
    )
...
def publish_flow_cancelled(
    session_id: str, flow_id: str, reason: str = "user_cancelled"
) -> None:
    """Publish a flow cancelled event."""
    data = FlowCancelledData(session_id=session_id, flow_id=flow_id, reason=reason)
    flow_cancelled.send(data=data)


def publish_display_value(
    session_id: str, step_id: str | None, path: str, value: Any, formatted_value: str
) -> None:
...
def publish_log_message(
    session_id: str, step_id: str | None, message: str, resolved_message: str
) -> None:
...
def publish_value_set(
    path: str, value: Any, old_value: Any = None, context_id: str | None = None
) -> None:
    """Publish a value set event (core engine)."""
    data = ValueSetData(
        path=path, value=value, old_value=old_value, context_id=context_id
    )
    value_set.send(data=data)


def publish_field_computed(
    path: str,
    computed_value: Any,
    source_fields: list[str],
    context_id: str | None = None,
...
def publish_step_executed(
    step_type: str, step_id: str, result: Any, context_id: str | None = None
) -> None:
    """Publish a step executed event (core engine)."""
    data = StepExecutedData(
        step_type=step_type, step_id=step_id, result=result, context_id=context_id
    )
