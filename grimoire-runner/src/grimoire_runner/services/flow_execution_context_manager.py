...
class NamespaceDataAccess(ABC):
    ...
    @abstractmethod
    def create_namespace(self, namespace_id: str, flow_id: str, execution_id: str) -> None:
    ...
    @abstractmethod
    def get_namespace_data(self, namespace_id: str) -> Optional[Dict[str, Any]]:
    ...
    @abstractmethod
    def get_namespace_variable(self, namespace_id: str, path: str, default: Any = None) -> Any:
        """Get a variable from a namespace."""
        pass
    
    @abstractmethod
    def initialize_namespace_variables(self, namespace_id: str, variables: Dict[str, Any]) -> None:
...
class FlowExecutionContextManager:
    ...
    def start_flow_execution(self, flow_id: str, variables: Optional[Dict[str, Any]] = None) -> FlowExecutionState:
        ...
        logger.info(f"Starting flow execution: {flow_id} (execution_id: {execution_id}, namespace: {namespace_id})")
        ...
        if variables:
            self.namespace_data_access.initialize_namespace_variables(namespace_id, variables.copy())
    ...
    def end_flow_execution(self) -> Optional[FlowExecutionState]:
        ...
        logger.info(f"Ending flow execution: {execution_state.flow_id} (namespace: {execution_state.namespace_id})")
        
        # Copy outputs to root level before cleanup
        self.namespace_data_access.copy_namespace_outputs_to_root(execution_state.namespace_id)
    ...
    def get_current_execution(self) -> Optional[FlowExecutionState]:
    ...
    def update_execution_step(self, step_id: str) -> None:
        ...
        if current_execution:
            ...
            logger.debug(f"Execution step updated: {step_id} (namespace: {current_execution.namespace_id})")
        else:
    ...
    def get_namespace_for_execution(self, execution_id: str) -> Optional[str]:
...
class DefaultNamespaceDataAccess(NamespaceDataAccess):
    ...
    def create_namespace(self, namespace_id: str, flow_id: str, execution_id: str) -> None:
        """Create a new namespace."""
        self.context.namespace_manager.create_flow_namespace(namespace_id, flow_id, execution_id)
    ...
    def set_namespace_variable(self, namespace_id: str, path: str, value: Any) -> None:
        """Set a variable in a namespace."""
        self.context.namespace_manager.set_flow_namespace_variable(namespace_id, path, value)
    
    def get_namespace_variable(self, namespace_id: str, path: str, default: Any = None) -> Any:
        """Get a variable from a namespace."""
        return self.context.namespace_manager.get_flow_namespace_variable(namespace_id, path, default)
    
    def initialize_namespace_variables(self, namespace_id: str, variables: Dict[str, Any]) -> None:
        """Initialize variables for a namespace."""
        self.context.namespace_manager.initialize_flow_namespace_variables(namespace_id, variables)
...
class ExecutionCheckpoint:
    ...
    @classmethod
    def create(cls, checkpoint_id: str, execution_state: FlowExecutionState, namespace_data: Dict[str, Any]) -> "ExecutionCheckpoint":
...
class CheckpointManager:
    ...
    def create_checkpoint(self, checkpoint_id: str, execution_state: FlowExecutionState) -> ExecutionCheckpoint:
        ...
        namespace_data = self.namespace_data_access.get_namespace_data(execution_state.namespace_id)
        
        # Create checkpoint
        checkpoint = ExecutionCheckpoint.create(checkpoint_id, execution_state, namespace_data or {})
        self.checkpoints[checkpoint_id] = checkpoint
        
        logger.info(f"Checkpoint created: {checkpoint_id} for execution {execution_state.execution_id}")
        return checkpoint
    ...
    def restore_checkpoint(self, checkpoint_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Restoring checkpoint: {checkpoint_id} for execution {checkpoint.execution_state.execution_id}")
