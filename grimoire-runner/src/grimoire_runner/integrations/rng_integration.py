...
class RNGIntegration:
    ...
    def generate_name(self, generator_id: str, **kwargs) -> str:
        ...
        if generator_id == "wyrdbound-rng":
            ...
            logger.debug(
                f"Attempting to generate name with corpus: {corpus}, max_length: {max_length}, algorithm: {algorithm}, segmenter: {segmenter}"
            )
            ...
            if corpus not in self.generators:
                logger.debug(
                    f"Generator for corpus '{corpus}' not loaded, loading now..."
                )
            ...
            if algorithm == "bayesian" and best_of is not None and best_of > 1:
                ...
                for i in range(best_of):
                    ...
                    logger.debug(
                        f"Generating name {i + 1}/{best_of} with params: {generation_params}"
                    )
                    ...
                    logger.debug(
                        f"Generated '{generated_name}' with probability: {name_probability}"
                    )
                    ...
                    if best_name is None or name_probability > best_probability:
                        ...
                        logger.debug(
                            f"New best name: '{best_name}' with probability: {best_probability}"
                        )

                logger.debug(
                    f"Selected best name '{best_name}' from {best_of} candidates with probability: {best_probability}"
                )
                return best_name
            else:
                ...
                logger.debug(
                    f"Calling generator.generate_name with params: {generation_params}"
                )
                generated_name = generator.generate_name(**generation_params)
                logger.debug(
                    f"Generated name '{generated_name}' using corpus '{corpus}' with params: {generation_params}"
                )
            ...
            raise RuntimeError(
                f"Unknown generator '{generator_id}' - only 'wyrdbound-rng' is supported"
            )
