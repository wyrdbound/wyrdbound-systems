...
class TestTableValidation:
    ...
    def test_table_roll_expression_validation(self, test_systems_dir):
        ...
        for table_id in tables_with_valid_rolls:
            ...
            assert len(errors) == 0, f"Table {table_id} should have valid roll expression"
...
class TestCompendiumTableIntegration:
    ...
    def test_cross_reference_validation(self, test_systems_dir):
        ...
        for entry_id, entry_value in equipment_table.entries.items():
            assert entry_value in gear_compendium.entries, f"Table entry '{entry_value}' not found in compendium"


class TestCompendiumTableErrorHandling:
    """Test error handling for malformed compendiums and tables."""

    def test_entry_type_validation_should_fail_for_mismatched_types(self, temp_system_dir):
        ...
        with open(table_file, 'w') as f:
            yaml.dump({
                ...
                "entries": {
                    ...
                    3: {"name": "Sword", "value": 100}  # Only this one matches item model
                }
            }, f)
        ...
        assert len(errors) >= 2, "Should detect entry type mismatches for string and integer entries"
        
        error_text = " ".join(errors)
        assert "entry_type" in error_text.lower(), "Errors should mention entry_type validation"
    ...
    def test_entry_type_validation_gap(self, test_systems_dir):
        ...
        for entry_value in equipment_table.entries.values():
            assert isinstance(entry_value, str)  # These are strings, not gear instances!
