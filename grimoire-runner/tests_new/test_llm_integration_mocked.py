...
class TestLLMIntegrationMocked:
    ...
    def test_llm_flow_execution_mocked(self, mock_generate):
        ...
        assert call_kwargs['prompt_template'] == "Create a character description for {{ character_name }}, a {{ character_class }}. Include their appearance, personality, and background."
    ...
    def test_template_syntax_validation(self):
        ...
        assert generate_step.prompt_data['character_name'] == "{{ inputs.character.name }}"
        assert generate_step.prompt_data['character_class'] == "{{ inputs.character.class }}"
...
class TestLLMErrorHandling:
    ...
    def test_llm_integration_fails_explicitly_when_dependencies_unavailable(self):
        ...
        with pytest.raises(ImportError, match="Ollama integration requested but langchain-community is not installed"):
            LLMIntegration(provider="ollama")

    @patch('grimoire_runner.integrations.llm_integration.LLMIntegration._setup_langchain')
    def test_llm_integration_fails_explicitly_on_setup_error(self, mock_setup):
        ...
        with pytest.raises(RuntimeError, match="Failed to initialize LLM integration: Setup failed"):
            LLMIntegration()
    ...
    def test_llm_flow_execution_with_mocked_integration(self):
        ...
        with patch.object(LLMIntegration, 'generate_content') as mock_generate:
            mock_generate.return_value = "A brave Fighter named TestCharacter stands ready for adventure."
