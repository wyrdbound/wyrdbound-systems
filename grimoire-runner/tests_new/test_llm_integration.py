...
class TestLLMIntegrationMocked:
    ...
    @patch(
        "grimoire_runner.integrations.llm_integration.LLMIntegration.generate_content",
    ...
    def test_llm_flow_execution_mocked(self, mock_generate):
        ...
        desc = result.outputs.get("character_with_description", {}).get(
            "description", None
        )
    ...
    @patch.object(
        LLMIntegration, "generate_content", return_value="Mocked character description"
    )
    ...
    def test_llm_integration_explicit_failure_behavior(self):
        ...
        with patch(
            "grimoire_runner.integrations.llm_integration.LANGCHAIN_CORE_AVAILABLE",
            False,
        ):
...
class TestLLMFlowIntegration:
    ...
    def test_llm_step_context_passing(self, mock_generate):
        ...
        assert (
            "Create a character description for" in prompt
            or "{{ character_name }}" in prompt
        )
...
def sample_character_context():
    ...
    return {"character_name": "TestHero", "character_class": "Warrior"}
